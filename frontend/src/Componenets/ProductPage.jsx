...
function ProductPage() {
    ...
    const fetchProducts = async () => {
        try {
            const res = await axios.get(`https://fixnparts.onrender.com/user/findProductAll`);
            console.log(res, "res")
    ...
    const addItem = async (productIddd) => {
        try {
            ...
            // return
            if (!finddAndCompare) {
                const dataa = await axios.post(`https://fixnparts.onrender.com/cartCreate`, { productId: productIddd }, {
                    headers: {
                        Authorization: `Bearer ${getToken?.token}`
                    }
                },)
                ...
                const dataa = await axios.put(`https://fixnparts.onrender.com/updateCartItem/${finddAndCompare._id}`, { quantity: finddAndCompare.quantity + 1 }, {
                    headers: {
                        Authorization: `Bearer ${getToken?.token}`
                    }
                },)
                ...
                const resss = await axios.get("https://fixnparts.onrender.com/findAllCart", {
                headers: {
                    Authorization: `Bearer ${getToken?.token}`
                }
            });
            console.log(resss,"resss")
            setCartCount(resss.data.body.length)
                ...
                {products.map((product) => (
                    <div className="col-md-4 mb-4" key={product._id}>
                        <div className="card h-100 shadow-sm">
                            <img
                                src={`https://fixnparts.onrender.com/image/productImage/${product.image}`}
