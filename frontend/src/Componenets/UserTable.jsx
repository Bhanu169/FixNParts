...
function UserTable() {
  ...
  const getData = async () => {
    try {
      const userData = await axios.get(`https://fixnparts.onrender.com/user/findUserAll`, {
        headers: {
    ...
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const dataa = await axios.delete(`https://fixnparts.onrender.com/user/deleteUserrr/${id}`)
          getData()
          Swal.fire({
            title: "Deleted!",
