...
function Cart() {
  ...
  try {
    ...
    console.log("Cart API response:", res.data); // <-- Add this line

    const cartItems = Array.isArray(res.data.body) ? res.data.body : [];
  ...
  const incFunction = async (id, quant) => {
    try {
      await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/updateCartItem/${id}`, { quantity: quant + 1 }, {
        headers: { Authorization: `Bearer ${getToken?.token}` }
      });
  ...
  const deleteUserrrData = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        await axios.delete(`${process.env.NEXT_PUBLIC_API_URL}/deleteSingleCartItem/${id}`, {
          headers: { Authorization: `Bearer ${getToken?.token}` }
        });
        fetchProducts();
                ...
                <div className="card mb-3 shadow-sm" key={item._id}>
                  <div className="card-body d-flex justify-content-between align-items-center">
                    <div className="d-flex align-items-center">
                      <img
                        src={`${process.env.NEXT_PUBLIC_API_URL}/image/productImage/${item?.productId?.image}`}
