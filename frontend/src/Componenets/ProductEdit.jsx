...
function ProductEdit() {
  ...
  const getData = async () => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/user/findProductById/${productId}`)
      setData(res.data.g)
      console.log(res)
    } catch (error) {
      console.error(error)
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      } await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/user/findProductByIdAndUpdate/${productId}`, dataa)
